...
class School21Client:
    ...
    def get_user_id(self) -> str:
        ...
        try:
            user_info = data["user"]["getCurrentUser"]
            return user_info["id"]
        except Exception as e:
            raise School21Error(
                f"Не смог распарсить информацию о пользователе из getCurrentUser: ошибка '{e}', ответ '{data}'")
    ...
    def get_reviewed_projects(self, user_id: str) -> list[tuple[str, str]]:
        ...
        try:
            cur_projects: list[dict] = data["student"]["getStudentCurrentProjects"]
            reviewed = []
            for project in cur_projects:
                if project["goalStatus"] == "P2P_EVALUATIONS" and project["goalId"]:
                    reviewed.append((project["goalId"], project["name"]))
            return reviewed
        except Exception as e:
            raise School21Error(f"Не смог распарсить информацию о текущих проектах из getStudentCurrentProjects: ошибка '{e}', ответ '{data}'")
